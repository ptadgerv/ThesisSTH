data{
 ...
 vector[N_ind] inf_eps_g;//epg infection intensity of each dataset
 vector<lower=0,upper=1>[N_ind] s_kk;//sensitivity of 1x1 KK
}

transformed data{
   vector[N_ind] inf_eps_g_t;//epg infection intensity of each
  vector<lower=0,upper=1>[N_ind] s_kk_t;//sensitivity of 1x1 KK
  s_kk_t=(s_kk+0.01)/1.02;
   inf_eps_g_t=(inf_eps_g+0.01)/8;
}
...
transformed parameters{
  ...
  vector<lower=0>[N_ind] beta;
    ...
    mu=inv_logit(a0+a1*exp(log(inf_eps_g_t)./a2));
    for(i in 1:N_ind){
    nu[i]=exp(b0+b1*log(inf_eps_g_t[i]));////worked with log for weird reasons
    }
    alpha=mu ./ nu;
